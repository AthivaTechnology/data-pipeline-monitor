Views.settingsMonitoring = async function (container) {
  ...
  const rows = pipelines.length === 0
    ? `<tr><td colspan="7" class="muted">No pipelines registered.</td></tr>`
    : pipelines.map((p) => `
        <tr>
  ...
  container.innerHTML = `
    ...
    <div class="table-wrap">
      <table>
        <thead>
          <tr>
            <th>Pipeline</th><th>Environment</th><th>Monitoring</th><th>Alerting</th>
            <th>Schedule</th><th>Output Freshness Source</th><th>Owner</th>
          </tr>
        </thead>
        <tbody>${rows}</tbody>
      </table>
